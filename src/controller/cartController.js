...
const createCart = async function (req, res) {
    try {
        ...
        let { productId, quantity ,cartId} = requestBody;
        ...
        if (findCartOfUser) {
            if(findCartOfUser._id.toString()!=cartId) return res.status(400).send({status:false,message:"cartId does not match please provide valid cart number"})
        ...
        for (let i = 0; i < items.length; i++) {
            if (items[i].productId == productId) {
                ...
                if (removeProduct == 0 || (items[i].quantity == 1 && removeProduct == 1)) {
                    ...
                    return res.status(200).send({ status: true, message: 'Success', data: removeCart })
                }
...
const deleteCartProducts = async function (req, res) {
    try {
        ...
        return res.status(204).send({ status: true, message: "Sucess", data: updateCartData })
